          ...
          stage('test image') {
            myEnv.withRun("${dockerOpt}") {c ->
              withEnv(rubyEnv + ["${dockerName.replaceAll('-','_').toUpperCase()}_ID=${c.id}", "LOCALHOST_ID=${c.id}", "DOCKER_IMAGE=${buildImage}"]) {
                ...
                sh "SPEC_OPTS='--format html --no-color --format RspecJunitFormatter --out spec/reports/${dockerName}.xml' rake spec:${dockerName}"
                sh "SPEC_OPTS='--format html --no-color --format RspecJunitFormatter --out spec/reports/localhost.xml' rake spec:localhost"
              }
            }
